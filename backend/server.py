...
from services.user_document_rag import UserDocumentRAGService
import uuid
import shutil
...
# Initialize file processor and user document RAG
file_processor = FileProcessor()
user_document_rag = UserDocumentRAGService()
...
# Initialize RAG service (singleton) with error handling
try:
    rag_service = RAGService()
    logger.info("✅ RAG service initialized successfully")
except Exception as e:
    logger.error(f"⚠️ RAG service initialization failed: {e}")
    rag_service = None  # Set to None if initialization fails

rag_router = APIRouter(prefix="/api/rag", tags=["RAG"])

@rag_router.get("/")
async def rag_root():
    """Get RAG API information"""
    return {
        "service": "Tesla RAG (Retrieval-Augmented Generation)",
        "status": "ready",
        "endpoints": {
            "process_documents": "POST /api/rag/process-documents?force_reprocess=false",
            "status": "GET /api/rag/status",
            "retrieve": "POST /api/rag/retrieve (Body: {query, top_k, filters})",
            "collection_info": "GET /api/rag/collection-info"
        }
    }

@rag_router.post("/process-documents")
@rag_router.get("/process-documents")  # Support both GET and POST
async def process_documents(force_reprocess: bool = False, use_async: bool = False):
    """
    Process all unprocessed documents from Tesla Knowledge Base folder
    
    Args:
        force_reprocess: If True, reprocess even completed documents (query parameter)
    ...
    if rag_service is None:
        raise HTTPException(
            status_code=503, 
            detail="RAG service is not available. Check server logs for initialization errors."
        )
    try:
        # Phase 3: Use async processing if requested
        if use_async:
            result = await rag_service.process_all_documents_async(force_reprocess=force_reprocess)
        else:
            result = rag_service.process_all_documents(force_reprocess=force_reprocess)
        return result
    except Exception as e:
...
async def get_rag_status():
    """
    Get RAG service status including processing and vector store info
...
@rag_router.get("/retrieve")  # Also support GET for easy browser testing
async def retrieve_documents(
    ...
    Args:
        ...
        filters: Optional metadata filters (JSON object, for POST requests)
...
@rag_router.get("/citations")
async def extract_citations_from_response(
    response_text: str,
    query: Optional[str] = None
):
    """
    Extract and verify citations from an AI response
    
    Args:
        response_text: AI-generated response text
        query: Original query (optional, for retrieval context)
    """
    if rag_service is None:
        raise HTTPException(
            status_code=503,
    ...
    try:
        # If query provided, retrieve documents for verification
        retrieved_docs = []
        if query:
            retrieved_docs = rag_service.retrieve_documents(query, top_k=10)
        
        # Extract and verify citations
        result = rag_service.extract_and_verify_citations(response_text, retrieved_docs)
        return result
    except Exception as e:
        logger.error(f"Error extracting citations: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error: {str(e)}")

@rag_router.post("/temporal-detect")
@rag_router.get("/temporal-detect")  # Support GET for easy testing
async def detect_temporal_query(query: str):
    """
    ...
    try:
        temporal_info = rag_service.temporal_reasoning.detect_temporal_query(query)
        return {
            "query": query,
            "temporal_info": temporal_info
        }
    except Exception as e:
        logger.error(f"Error detecting temporal query: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error: {str(e)}")

@rag_router.get("/cache-stats")
async def get_cache_stats():
    ...
    try:
        stats = rag_service.cache_service.get_cache_stats()
        return stats
    except Exception as e:
        logger.error(f"Error getting cache stats: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Error: {str(e)}")

@rag_router.get("/collection-info")
...
# Financial Report Analyzer Router
fra_router = APIRouter(prefix="/api/fra", tags=["Financial Report Analyzer"])

@fra_router.post("/upload")
async def upload_document(
    file: UploadFile = File(...),
    user_id: Optional[str] = Form(None)
):
    """
    Upload and process a document (PDF, Excel, CSV, Image)
    
    Returns:
        File processing result with file_id
    """
    try:
        ...
        file_path = file_processor.upload_dir / saved_filename
        ...
        # Process file
        extracted_content = file_processor.process_file(
            str(file_path),
            file.filename,
        ...
        if not extracted_content.get("success"):
            return JSONResponse(
                ...
                content={
                    "success": False,
                    "error": extracted_content.get("error", "Failed to process file"),
                    "file_id": file_id
                }
            )
        
        # Store in vector DB
        processing_result = user_document_rag.process_uploaded_document(
            file_id=file_id,
        ...
        if not processing_result.get("success"):
            return JSONResponse(
                ...
                content={
                    "success": False,
                    "error": processing_result.get("error", "Failed to store in vector DB"),
                    "file_id": file_id
                }
            )
...
@fra_router.post("/summarize")
async def summarize_document(request: SummarizeRequest):
    """
    Generate summary and insights for an uploaded document
    ...
    try:
        ...
        query_embedding = user_document_rag.embedding_service.generate_embedding("summary key insights")
        chunks = user_document_rag.vector_store.search(
        ...
        summary_query = f"Provide a comprehensive executive summary and key insights for this document:\n\n{document_text[:8000]}"  # Limit length
        
        if enhanced_agent_orchestrator.openai_client:
            response = await enhanced_agent_orchestrator.handle_financial_modeling_query(
                query=summary_query,
                classification=None,
            ...
            return {
                "success": True,
                "file_id": file_id,
                "summary": document_text[:1000] + "...",
                "key_insights": ["Document processed successfully"],
                "recommendations": [],
                "chunks_used": len(chunks),
                "note": "OpenAI not available, returning basic summary"
            }
...
async def ask_question(request: AskRequest):
    ...
    try:
        ...
        retrieved_chunks = user_document_rag.retrieve_documents(
            query=question,
            file_id=file_id,
            top_k=10,
            user_id=user_id
        ...
        # Include more chunks for better context (up to 10)
        context_text = "\n\n".join([
            f"[Document Excerpt {i+1}]\n{chunk.get('text', '')}"
            for i, chunk in enumerate(retrieved_chunks[:10])
        ])
        
        # Build query with document context embedded
        # The prompt builder will be instructed to ONLY use this content
        full_query = f"{question}\n\n=== UPLOADED DOCUMENT CONTENT ===\n\nThe following excerpts are from the user's uploaded document. Base your answer ONLY on this content:\n\n{context_text}\n\n=== END OF DOCUMENT CONTENT ==="
        ...
        if enhanced_agent_orchestrator.openai_client:
            ...
            # Extract and verify citations
            citations_result = {}
            if enhanced_agent_orchestrator.rag_service:
                answer_text = response.get("executive_summary", "")
                citations_result = enhanced_agent_orchestrator.rag_service.extract_and_verify_citations(
                    answer_text,
                    retrieved_chunks
                )
            ...
            return {
                ...
                "file_id": file_id,
                "question": question,
                "answer": response.get("executive_summary", ""),
                "key_insights": response.get("key_insights", []),
...
@fra_router.get("/documents")
async def list_documents(user_id: Optional[str] = None):
    """List all uploaded documents"""
    try:
        # Get collection info
        collection_info = user_document_rag.vector_store.get_collection_info()
        
        return {
            "success": True,
            "collection": collection_info,
            "note": "Document listing by metadata requires additional implementation"
        }
...
@fra_router.delete("/documents/{file_id}")
async def delete_document(file_id: str):
    """Delete an uploaded document"""
    try:
        result = user_document_rag.delete_document(file_id)
        return result
